...
export const isHex = (s: string) => /^[0-9a-fA-F]+$/.test(s.replace(/^0x/, ""));
export const strip0x = (h: string) => (h.startsWith("0x") ? h.slice(2) : h);

export const Q = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141n;
export const HALF_Q = (Q >> 1n) + 1n;
 ...
 */
export const concat = (...arrs: Uint8Array[]) => {
 ...
 */
export const i2osp = (value: number, length: number) => {
 ...
 */
export const os2ip = (bytes: Uint8Array): bigint => {
    let res = 0n;
 ...
 */
export function to32(b: Uint8Array): Uint8Array {
 ...
 */
export function compressedFromXY(pxHex: string, pyHex: string): Uint8Array {
 ...
 */
export function expand_message_xmd_sha256(msg: Uint8Array, DST: Uint8Array, lenInBytes: number): Uint8Array {
 ...
 */
export function dst_xmd_sha256(parts: Uint8Array[]): Uint8Array {
 ...
 */
export function hash_to_scalar(msg: Uint8Array, DST: Uint8Array): bigint {
 ...
 */
export const CONTEXT = te.encode("FROST-secp256k1-SHA256-v1");
...
export const bytesToBigInt = (b: Uint8Array | Buffer): bigint => BigInt("0x" + Buffer.from(b).toString("hex"));
...
export function sign(m: string, x: Uint8Array) {
